...
import os
import numpy as np
from garbage.model import make_prediction
...
def mouseclick(events, x, y, flags, params):
    ...
    if events == cv2.EVENT_LBUTTONDOWN:
        ...
        if len(current_pos) == 4:
            ...
            save_img(img=cv2.imread('parking.png'), 
                             points=current_pos,
                             id=(get_next_id()-1))
            current_pos = []
...
def initial_prediction():
    for pos in positionList:
        prediction = make_prediction(image=cropped_img(img=cv2.imread('parking.png'), points=pos['points']))
